...
NanoClaw provides that same core functionality, but in a codebase small enough to understand: one process and a handful of files. Codex-backed agents run in their own Linux containers with filesystem isolation, not merely behind permission checks.
...
`nanoclaw.sh` walks you from a fresh machine to a named agent you can message. It installs Node, pnpm, and Docker if missing, checks your Codex login, builds the agent container, and pairs your first channel (Telegram, Discord, WhatsApp, or a local CLI). If a step fails, Codex can help diagnose the logs and suggest a recovery command.
...
**Codex-first provider.** This fork uses OpenAI Codex by default through the Codex CLI and `codex app-server`, with ChatGPT login or `OPENAI_API_KEY` authentication. Claude remains available as an explicit provider for users who configure it, and OpenCode / Ollama can still be added from the provider skills.
...
- **Scheduled tasks** — recurring jobs that run your configured agent provider and can message you back
...
NanoClaw doesn't use configuration sprawl. To make changes, tell Codex what you want:
...
- [Codex CLI](https://help.openai.com/en/articles/11096431-openai-codex-cli-getting-started) with ChatGPT login (`codex login`) or `OPENAI_API_KEY`

## Architecture

```
messaging apps → host process (router) → inbound.db → container (Bun, Codex app-server) → outbound.db → host process (delivery) → messaging apps
```

A single Node host orchestrates per-session agent containers. When a message arrives, the host routes it via the entity model (user → messaging group → agent group → session), writes it to the session's `inbound.db`, and wakes the container. The agent-runner inside the container polls `inbound.db`, runs Codex by default, and writes responses to `outbound.db`. The host polls `outbound.db` and delivers back through the channel adapter.
...
- `src/providers/` — host-side provider config (`codex` default, `claude` optional)
...
For Codex API-key or compatible-endpoint experiments, use `.env`:
...
Ask Codex. "Why isn't the scheduler running?" "What's in the recent logs?" "Why did this message not get a response?" That's the AI-native approach that underlies this fork.

**Why isn't the setup working for me?**

If a step fails, `nanoclaw.sh` can ask Codex to inspect the setup logs and suggest a recovery command. If Codex identifies an issue likely to affect other users, open a PR against the relevant setup step or skill.
