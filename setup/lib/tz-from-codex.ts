...
export async function resolveTimezoneViaCodex(input: string): Promise<string | null> {
  ...
  const reply = await queryCodex(buildPrompt(input));
...
function buildPrompt(input: string): string {
  return [
    "Convert the user's description of where they are into a single IANA timezone identifier.",
    'Respond with ONLY the IANA string on a single line, nothing else.',
    'If you cannot determine a zone with reasonable confidence, reply with exactly: UNKNOWN',
    '',
...
function queryCodex(prompt: string): Promise<string | null> {
  return new Promise((resolve) => {
    const child = spawn(
      'codex',
      ['exec', '--sandbox', 'read-only', '--ask-for-approval', 'never', '--color', 'never', '-'],
      {
